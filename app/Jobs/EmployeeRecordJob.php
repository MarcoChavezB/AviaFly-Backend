...
public function handle()
{
    try {
        ...
        $records = DB::table('employees as e')
            ...
            ->where('cr.type', 'hora de comida')  // Último registro debe ser "hora de comida"
            ->whereNotExists(function ($query) {
                $query->select(DB::raw(1))
                      ...
                      ->where('cr2.type', 'fin de comida'); // No debe haber "fin de comida"
            })
        ...
        foreach ($records as $lastRecord) {
            ...
            if ($currentTime > $lastMealTimePlus40) {
                ...
                $this->sendWarning($lastRecord, $currentTime->diffInMinutes($lastArrivalTime));
            } else {
...
private function sendWarning($lastRecord, $minutesPassed)
{
    // Utilizar el último registro para acceder a los datos del empleado
    $employee = (object) [
